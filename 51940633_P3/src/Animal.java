...
public abstract class Animal implements Cloneable, Comparable<Animal>, Serializable {
	 ...
	 * A: Animal should be abstract to prevent creating animals of unknown types
	 * (i.e. that are not Cows, Llamas, or Chicken).
	 * Animal just provides a base for its subclasses which inherit the implemented
	 * methods and are forced to implement the abstract methods.
	 */
	private String name;
	 ...
	 * Since we don't have unknown animals with unknown sounds, it doesn't make
	 * sense to implement 'sound()' in Animal
	 * why not just delete it from Animal?
	 * (a) we want to force all subclasses to include a sound method.
	 * (b) having sound() defined in the superclass will allow calling it using a
	...
	public int compareTo(Animal otherAnimal) {
		if (this.energy < otherAnimal.energy)
			return -1;
		else if (this.energy > otherAnimal.energy)
			return 1;
		else
			return 0;
		/*
		 ...
		 * //code above is ok but gets only 50% of the mark - since casting will lose
		 * precision (e.g. if difference is < 0.5, method will return 0 indicating the
